...
d_x=alpha*dt/dx**2           # Diffusion number
d_y=alpha*dt/dy**2           # Diffusion number
...
while err1>cr1:
    ...
    for i in range(1,m):
        for j in range(1,n):
            p[i,j]=(  p[i,j]+d_x*(p[i-1,j]-p[i,j]+p[i+1,j])   +d_y*(p[i,j-1]-p[i,j]+p[i,j+1])  )/(1+d_x+d_y)
            
    # step 2
    for i in range(m-1,0,-1):
        for j in range(n-1,0,-1):
            q[i,j]=(  q[i,j]+d_x*(q[i-1,j]-q[i,j]+q[i+1,j])   +d_y*(q[i,j-1]-q[i,j]+q[i,j+1])  )/(1+d_x+d_y)
...
print(' x Diffusion number:{:8.3f}'.format(Fo_x))
print(' y Diffusion number:{:8.3f}'.format(Fo_y))
